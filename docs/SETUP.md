...
For legacy, asset, or repository archaeology, begin with the
[archaeology guide](archaeology/README.md).
...
```sh
python3.12 tools/ci/run_repository_checks.py repository-policy \
  --baseline-ref origin/dev --candidate-ref CANDIDATE_TREE
python3.12 tools/ci/run_repository_checks.py tests
```
...
2. Install and authenticate Git LFS when the project uses configured binary
   asset formats.
3. Initialize the GameMaker project under `project/` when needed, extend its
   tests without modifying the pinned GMTL snapshot, and pin the required
   GM-Testing-Library release. Connect game-specific hosted runners and
   secrets through the [CI extension procedure](CI.md).
...
boundary for its exact resource roots and file inventory. The lock's roots,
inventory, SHA-256 hashes, and sizes must all verify before a locked file is
exempted from the source line limit or Format whitespace findings.
