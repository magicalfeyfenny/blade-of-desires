...
class CiTestContractTests(unittest.TestCase):
    ...
    def test_format_preserves_verified_gmtl_whitespace_boundary(self):
        """Keep Blade's lock-aware formatting check bound to the actual PR refs."""
        job = self.jobs["format"]
        checks = [
            step for step in job["steps"]
            if "tools/ci/check_format.py" in step.get("run", "")
        ]
        self.assertEqual(len(checks), 1)
        check = checks[0]
        self.assertEqual(
            shlex.split(check["run"].replace("\\\n", " ")),
            ["python3", "tools/ci/check_format.py", "--base-ref", "$BASE_SHA",
             "--head-ref", "$HEAD_SHA"],
        )
