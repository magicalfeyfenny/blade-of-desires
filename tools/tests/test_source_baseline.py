...
from tools.tests.gmtl_fixture import write_gmtl_fixture
...
class SourceBaselineCliTests(SourceBaselineFixture):
    def setUp(self):
        ...
        for name in ("check_repo.py", "candidate_git.py", "storage_policy.py", "gmtl_lock.py"):
    ...
    def test_policy_edit_preserves_included_directory_symlink(self):
        """Historical evaluation must keep Included File links instead of copying folders."""
        included = self.root / "project/datafiles/source"
        included.parent.mkdir(parents=True)
        included.symlink_to("../../source", target_is_directory=True)
        self.commit_baseline()
        policy = self.root / "PROJECT_POLICY.toml"
        policy.write_text(policy.read_text() + "\n# Policy contract edit\n")
        result = self.run_checker()
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertTrue(included.is_symlink())
        self.assertEqual(included.readlink().as_posix(), "../../source")

    def test_gmtl_contract_edits_disable_partial_source_reductions(self):
        """Lock ownership and its validator retain the same conservative boundary."""
        for name in ("project/gmtl.lock.json", "tools/ci/gmtl_lock.py"):
    ...
    def disable_current_structure_check(self):
        ...
        self.checker.write_text(
            self.checker.read_text().replace(
                ...
                '\ndef validate_structure(root, policy, files, errors, line_limit_exemptions=frozenset()):\n'
                '    pass\n\nif __name__ == "__main__":',
            ),
            encoding="utf-8",
        )
